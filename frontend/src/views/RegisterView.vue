...
async function handleSubmit() {
  ...
  if (success) {
    ...
    // Очищаем форму
    form.value = { full_name: '', login: '', email: '', phone: '', password: '' }
  ...
  <div class="flex min-h-screen items-center justify-center bg-gradient-to-br from-indigo-600 via-indigo-500 to-purple-600 p-4">
    <div class="animate-fade-in w-full max-w-lg">
      <div class="mb-8 text-center">
        ...
        <p class="mt-1 text-indigo-100">Создание учётной записи</p>
      </div>

      <div class="rounded-2xl border border-white/20 bg-white p-8 shadow-2xl backdrop-blur">
        <h2 class="mb-6 text-center text-2xl font-bold text-gray-900">Регистрация</h2>
        ...
        <div v-if="successMessage" class="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700">
          {{ successMessage }}
        </div>
        ...
        <p class="mt-6 text-center text-sm text-gray-500">
          Уже есть аккаунт?
          <router-link to="/login" class="font-semibold text-indigo-600 transition-colors hover:text-indigo-700">
            Войти
          </router-link>
        </p>
      </div>
